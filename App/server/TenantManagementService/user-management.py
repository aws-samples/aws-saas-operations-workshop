...
def create_tenant_admin_user(event, context):
    ...
    tenant_identity_pool_id = os.environ['TENANT_IDENTITY_POOL_ID']
    ...
    if (tenant_details['dedicatedTenancy'] == 'true'):
        ...
        #region, tenant_id, userpool_id, userpool_client_id, user_role_arn
        identity_pool_response = user_mgmt.create_identity_pool(
            region,
            tenant_id,
            user_pool_id,
            app_client_id,
        ...
        identity_pool_id = tenant_identity_pool_id
    ...
    response = {"userPoolId": user_pool_id, "identityPoolId": identity_pool_id, "appClientId": app_client_id, "tenantAdminUserName": tenant_admin_user_name}
...
def create_user(event, context):
    ...
    logger.log_message(event, "Request received to create new user")
    ...
    if (auth_manager.isTenantAdmin(user_role) or auth_manager.isSystemAdmin(user_role)):
        ...
        logger.log_message(event, response)
        user_mgmt = UserManagement()
        ...
        logger.log_message(event, "Request completed to create new user ")
        return utils.create_success_response("New user created")
    else:
        logger.log_message(event, "Request completed as unauthorized. Only tenant admin or system admin can create user!")        
...
def get_users(event, context):
    ...
    logger.log_message(event, "Request received to get user")
    ...
    if (auth_manager.isTenantAdmin(user_role) or auth_manager.isSystemAdmin(user_role)):
        ...
        metrics_manager.record_metric(event, "Numberofusers", "Count", num_of_users)
        if (num_of_users > 0):
        ...
        logger.log_message(event, "Request completed as unauthorized.")        
        return utils.create_unauthorized_response()
...
def get_user(event, context):
    ...
    if (auth_manager.isTenantUser(user_role) and user_name != requesting_user_name):        
        logger.log_message(event, "Request completed as unauthorized. User can only get its information.")        
        return utils.create_unauthorized_response()
    else:
        ...
        if(not auth_manager.isSystemAdmin(user_role) and user_info.tenant_id!=tenant_id):
            logger.log_message(event, "Request completed as unauthorized. Users in other tenants cannot be accessed")
            return utils.create_unauthorized_response()
        else:
            logger.log_message(event, "Request completed to get new user ")
...
def update_user(event, context):
    ...
    if (auth_manager.isTenantUser(user_role)):                
        logger.log_message(event, "Request completed as unauthorized. Only tenant admin or system admin can update user!")        
        return utils.create_unauthorized_response()
    else:
        user_info = get_user_info(event, user_pool_id, user_name)
        if(not auth_manager.isSystemAdmin(user_role) and user_info.tenant_id!=tenant_id):
            ...
            logger.log_message(event, response)
            logger.log_message(event, "Request completed to update user ")
...
def disable_user(event, context):
    ...
    logger.log_message(event, "Request received to disable new user")
    ...
    if (auth_manager.isTenantAdmin(user_role) or auth_manager.isSystemAdmin(user_role)):
        ...
        if(not auth_manager.isSystemAdmin(user_role) and user_info.tenant_id!=tenant_id):
            ...
            logger.log_message(event, response)
            logger.log_message(event, "Request completed to disable new user ")
            return utils.create_success_response("User disabled")
    else:
        logger.log_message(event, "Request completed as unauthorized. Only tenant admin or system admin can disable user!")        
        return utils.create_unauthorized_response()  
...
def get_user_info(event, user_pool_id, user_name):
    ...
    for attr in response["UserAttributes"]:
        ...
        if(attr["Name"] == "email"):
            user_info.email = attr["Value"] 
    logger.log_message(event, user_info)
    return user_info    
...
class UserManagement:
    def create_user_pool(self, tenant_id):
        ...
        response = client.create_user_pool(
            PoolName= tenant_id + '-SaaSOperationsUserPool',
    ...
    def create_user_pool_client(self, user_pool_id):
        ...
        response = client.create_user_pool_client(
            ...
            ExplicitAuthFlows=[
                'ALLOW_REFRESH_TOKEN_AUTH','ALLOW_CUSTOM_AUTH','ALLOW_USER_SRP_AUTH','ALLOW_USER_PASSWORD_AUTH'
            ],
            IdTokenValidity=12
        )
        return response
    ...
    def create_user_pool_domain(self, user_pool_id, tenant_id):
        response = client.create_user_pool_domain(
            Domain= tenant_id + '-saasoperations',
            UserPoolId=user_pool_id
        )
        return response
    ...
    def create_identity_pool(self, region, tenant_id, userpool_id, userpool_client_id, user_role_arn):
        ...
        idpool_response = id_client.create_identity_pool(
            IdentityPoolName = tenant_id + '-identitypool',
            AllowUnauthenticatedIdentities = False,
            CognitoIdentityProviders=[
                {
                    'ProviderName': userpool_provider_name,
        ...
        client.create_identity_provider(
            ...
            ProviderDetails={
                ...
                "oidc_issuer": userpool_provider_url
            }
        )

        id_client.set_principal_tag_attribute_map(
            IdentityPoolId=identity_pool_id,
            IdentityProviderName='cognito-idp.'+region+'.amazonaws.com/'+userpool_id,
            UseDefaults=False,
            PrincipalTags={
