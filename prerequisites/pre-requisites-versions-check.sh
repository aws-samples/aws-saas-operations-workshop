...
if [[ $? -eq 1 ]]; then
    ...
    SUMMARY+="* PASS : Python version $PYTHON_VERSION installed"$'\n'
fi
echo ""
...
if [[ $? -ne 0 ]]; then
    ...
    if [ -z $CLOUD9_INSTANCE ]; then
        echo "ACTION REQUIRED: Looks like Cloud9 instance with m5.large instance type is missing. Please create one!!"
        SUMMARY+="* ACTION REQUIRED: Looks like Cloud9 instance with m5.large instance type is missing. Please create one!!"$'\n'
    else
        SUMMARY+="* PASS : Has required m5.large instance type"$'\n' 
...
if [[ $? -eq 1 ]]; then
    ...
    SUMMARY+="* PASS : Sam cli version $SAM_VERSION installed"$'\n'
fi
echo ""
...
if [[ $? -eq 1 ]]; then
    ...
    SUMMARY+="* PASS : Node version $NODE_VERSION installed"$'\n'
fi
echo ""
...
if [[ $? -eq 1 ]]; then
    ...
    SUMMARY+="* PASS : CDK version $CDK_VERSION installed"$'\n'
fi
echo ""

echo "Checking SaaS application deployment"
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "${ADMIN_APIGATEWAYURL}/tenant/init/PooledTenant1")

echo $RESPONSE

if [[ RESPONSE -ne 200 ]]; then
    echo "ACTION REQUIRED: 11 SaaS Application deployment verification failed"
    SUMMARY+="* ACTION REQUIRED: SaaS Application deployment verification failed"$'\n'
else 

    RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "${ADMIN_APIGATEWAYURL}/tenant/init/SiloedTenant1")

    if [[ RESPONSE -ne 200 ]]; then
        echo "ACTION REQUIRED: 22 SaaS Application deployment verification failed"
        SUMMARY+="* ACTION REQUIRED: SaaS Application deployment verification failed"$'\n'
    else
        SUMMARY+="* PASS : SaaS Application deployment verification success"$'\n'
    fi
fi
echo ""
